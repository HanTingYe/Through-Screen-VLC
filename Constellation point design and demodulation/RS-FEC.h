...
/* GF tables pre-calculated for 0x11d primitive polynomial  x^8+x^4+x^3+x^2+1*/
...
public:
     ...
     * @param *dst - output buffer for ecc     (ecc_length size at least) */
     void EncodeBlock(const void* src, void* dst) {
        assert(msg_length + ecc_length < 256); //Debug dedicated function
        ...
        for(uint8_t i = 0; i < msg_length; i++){
            coef = msg_out->at(i); //remainder of division
            if(coef != 0){
                for(uint32_t j = 1; j < gen->length; j++){			//gen->length=ecc_length+1	
                    //uint8_t test_temp = 0; //Test
					//test_temp = msg_out->at(i+j);
