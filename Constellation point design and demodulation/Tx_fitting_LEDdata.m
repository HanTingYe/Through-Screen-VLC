...
figure(18)
plot(PWM,Normalized_R,'ko-');hold on
plot(PWM,Normalized_G,'ko-');hold on
plot(PWM,Normalized_B,'ko-');hold on
...
CSK_x_4(1)=mean(x_primary);
CSK_y_4(1)=mean(y_primary);
CSK_x_4(2)=x_primary(2);
CSK_y_4(2)=y_primary(2);
CSK_x_4(3)=x_primary(3);
CSK_y_4(3)=y_primary(3);
CSK_x_4(4)=x_primary(1);
CSK_y_4(4)=y_primary(1);
...
CSK_x_8=[];
CSK_y_8=[];
B_x=(x_primary(1)+x_primary(2))/2;
B_y=(y_primary(1)+y_primary(2))/2;
C_x=(x_primary(2)+x_primary(3))/2;
C_y=(y_primary(2)+y_primary(3))/2;

CSK_x_8(1) = x_primary(2);
CSK_y_8(1) = y_primary(2);
CSK_x_8(2) = (2*x_primary(2)+x_primary(3))/3;
CSK_y_8(2) = (2*y_primary(2)+y_primary(3))/3;
CSK_x_8(3) = (2*x_primary(2)+x_primary(1))/3;
CSK_y_8(3) = (2*y_primary(2)+y_primary(1))/3;
CSK_x_8(5) = x_primary(3);
CSK_y_8(5) = y_primary(3);
CSK_x_8(7) = (x_primary(1)+x_primary(3))/2;
CSK_y_8(7) = (y_primary(1)+y_primary(3))/2;
CSK_x_8(8) = x_primary(1);
CSK_y_8(8) = y_primary(1);
CSK_x_8(4) = (CSK_x_8(8)+CSK_x_8(7)+B_x)/3;
CSK_y_8(4) = (CSK_y_8(8)+CSK_y_8(7)+B_y)/3;
CSK_x_8(6) = (CSK_x_8(5)+CSK_x_8(7)+C_x)/3;
CSK_y_8(6) = (CSK_y_8(5)+CSK_y_8(7)+C_y)/3;
...
hold on
scatter(x_primary,y_primary,120,'o','black')
plot([x_primary; x_primary],[y_primary; y_primary],'k')
scatter(CSK_x_4,CSK_y_4,120,'^','k');
scatter(CSK_x_8,CSK_y_8,120,'*','k');
grid off;
set(gca,'FontSize',16,'FontName','Arial');
...
for i=M:-1:2
    for j=1:i-1
        d_all(i-1,j)=sqrt((CSK_x(i)-CSK_x(j))^2+(CSK_y(i)-CSK_y(j))^2)
    end
end
...
yscreen=CSK_y(1);
for i=1:M
    k(i)=(CSK_y(i)-yscreen)/(CSK_x(i)-xscreen);
end
%d_all=zeros(M-1,M-1);
for i=M:-1:2
    for j=2:i-1
        theta_all(i-1,j)=pi-atan(abs((k(i)-k(j))/(1+k(i)*k(j))))
    end
end
