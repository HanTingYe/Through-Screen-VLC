...
while(t<interval)
    channel_label = [channel_label, convertStringsToChars(readline(s))];  
    data = [data, str2num(readline(s))];  
    t = t+passo
end


color='Test_screen_intensity_demo';
save([color]);
delete (s);
...
for ii=premble_index:length_min
    ...
    if count_flag == 2 %
        ...
        symbol_index = [symbol_index, ii-1];
        symbol_flag=0;
        if symbol_received(ii)==temp_com && symbol_received(ii-1)==temp_com  
            count_flag=-1;
        else
            count_flag=0;
        end
...
for ii=1:fix(length(message_encoded_ASCII)/(msglen+ecclen))
    temp=[];
    temp=Decode(message_encoded_ASCII((msglen+ecclen)*(ii-1)+1:(msglen+ecclen)*ii));
    if length(temp)>=msglen
        message_decoded=[message_decoded temp(1:msglen)];% Input: ASCII code of message
    end
end

%message_decoded=Decode(message_encoded_ASCII);% Input: ASCII code of message

%outputs

for kk=1:fix(length(message_decoded)/msglen)
    cellArrayText{kk} = sprintf('%s\n%s', message_decoded(msglen*(kk-1)+1:msglen*kk));
    textarea = uitextarea('Value',cellArrayText);
    textarea.Position(3:4) = [400 300];
