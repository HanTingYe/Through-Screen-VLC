...
load Section3_intensity_0_30000_revised.mat

%load Original_4CSK_2.mat

%delte 628th point for intensity 31
% x(628)=[];
% y(628)=[];
...
hold on
scatter(x,y,48,'black','.');
scatter(x_screen,y_screen,48,'*','black')
grid off;
axis off;
ylim([0 0.4]);
xlim([0 0.8])
...
for j=1:1:length_min
    d(j)=sqrt((x(j)-x_sent)^2+(y(j)-y_sent)^2)/sqrt((x_screen-x_sent)^2+(y_screen-y_sent)^2);    
end
d(find(isnan(d)))=[];
figure(8)
cdfplot(d);
%% time relationship
rgb_8 = []
rgb_8 = xyz2rgb([x(1:100)' y(1:100)' (1-x(1:100)-y(1:100))']);
figure(7)
colorSwatches(rgb_8(:,:));
%set(gca,'yticklabel',[]);
%set(gca,'ytick',[])
...
xlabel('Samples')
ylabel('Color')
%save Section3_intensity_31_30000_revised.mat
...
% X_recover(ii)=X(error_index(ii))+4000;
% Y_recover(ii)=Y(error_index(ii))-28000;
% Z_recover(ii)=Z(error_index(ii))-3000;
% end
% 
...
% %display
% for ii=1:length(error_index)
% X(error_index(ii))=X(error_index(ii))+4000;
% Y(error_index(ii))=Y(error_index(ii))-28000;
% Z(error_index(ii))=Z(error_index(ii))-3000;
% end
% 
% for jj=1:1:length(error_index)
