...
load Results_RS_16CSK_25_15.mat
...
normalcode_dec=0:M-1;
normalcode_bin_char=dec2bin(normalcode_dec,4);
...
graycode_bin = gen_gray_code(4);
graycode_dec = reshape([8 4 2 1]*reshape(graycode_bin.',4,[]),size(graycode_bin)./[1 4]);
% 8CSK
...
CSK_x = [0.314, 0.204, 0.358, 0.246, 0.182, 0.338, 0.182, 0.302, 0.294, 0.425, 0.464, 0.529, 0.522, 0.634, 0.589, 0.447];
CSK_y = [0.586, 0.592, 0.507, 0.508, 0.432, 0.431, 0.303, 0.302, 0.162, 0.356, 0.217, 0.294, 0.391, 0.296, 0.356, 0.474];
...
CSK_x_screen = 0.435;%0.4400
CSK_y_screen = 0.525;%0.5000
symbol=[];
for ii=1:length_min
    delta_d=[];
    for jj=1:M
...
screen_threshold=13000;
symbol_Txscreen_index_test = find(X(1:length_min)>screen_threshold);
symbol_Tx_index = find(X(1:length_min)<=screen_threshold);

screen_threshold_X=10500;%mean(X(premble_index:length_min))
screen_threshold_Y=8500;%mean(X(premble_index:length_min))
for ii=1:length_min
    if X(ii)>screen_threshold_X && Y(ii)>screen_threshold_Y
       symbol_Txscreen_index = [symbol_Txscreen_index ii];
    else
        symbol_Tx_index = [symbol_Tx_index ii];
    end
end
...
T_screen1=diff(symbol_Txscreen_index);
T_screen2=diff(symbol_Txscreen_index_test);
%%  slope detection
% symbol_test=[];
...
%         symbol_received(symbol_Txscreen_index(ii))=8;
%     end
% end
% 
...
%%  slope detection V2 5.25% for intensity 15

% symbol_test=[];
% symbol_test=symbol_received;
% %CSK_x_underscreen = [0.428, 0.430, 0.415, 0.420, 0.415, 0.428, 0.406, 0.426, 0.424, 0.442, 0.449, 0.454, 0.452, 0.471, 0.464, 0.443]
% %CSK_y_underscreen = [0.526, 0.517, 0.518, 0.509, 0.504, 0.504, 0.483, 0.486, 0.463, 0.496, 0.473, 0.484, 0.5, 0.485, 0.496, 0.513]
% %CSK_x_underscreen = [0.411, 0.401, 0.422, 0.399, 0.401, 0.417, 0.384, 0.411, 0.406, 0.433, 0.442, 0.454, 0.454, 0.476, 0.468, 0.44]
% %CSK_y_underscreen = [0.541, 0.536, 0.424, 0.524, 0.515, 0.509, 0.480, 0.483, 0.454, 0.492, 0.464, 0.479, 0.499, 0.479, 0.493, 0.516]
% 
% CSK_x_underscreen = [0.411, 0.401, 0.422, 0.399, 0.401, 0.417, 0.384, 0.411, 0.406, 0.433, 0.442, 0.454, 0.454, 0.471, 0.464, 0.44]
% CSK_y_underscreen = [0.541, 0.536, 0.525, 0.524, 0.515, 0.509, 0.480, 0.483, 0.454, 0.492, 0.464, 0.479, 0.499, 0.485, 0.496, 0.516]
% 
% Slope_threshold=0.075;%0.054
% y_threshold=0.525;%0.5
% CSK_high_y = find(CSK_y > y_threshold);
% CSK_low_y = find(CSK_y < y_threshold);
% CSK_high_y(CSK_high_y==16)=[];
% 
% for ii=1:length(symbol_Txscreen_index)
%     if symbol_Txscreen_index(ii)==1256
%         tttt=1;
%     end
%     %~IsPointInTriangle_16(x(symbol_Txscreen_index(ii)),y(symbol_Txscreen_index(ii)))
...
CSK_x_underscreen = [0.411, 0.401, 0.422, 0.399, 0.401, 0.417, 0.384, 0.411, 0.406, 0.433, 0.442, 0.454, 0.454, 0.476, 0.468, 0.44]
CSK_y_underscreen = [0.541, 0.536, 0.525, 0.524, 0.515, 0.509, 0.480, 0.483, 0.454, 0.492, 0.464, 0.479, 0.499, 0.479, 0.493, 0.516]
Slope_threshold=0.075;%0.054
y_threshold=0.525;%0.5
CSK_high_y = find(CSK_y > y_threshold);
CSK_low_y = find(CSK_y < y_threshold);
...
%     end
    ...
    if sqrt((x(symbol_Txscreen_index(ii))-CSK_x_underscreen(M))^2+(y(symbol_Txscreen_index(ii))-CSK_y_underscreen(M))^2)>Slope_threshold
        delta_k=[];
        for jj=1:M-1
            delta_k(jj)=abs((y(symbol_Txscreen_index(ii))-CSK_y_screen)/abs(x(symbol_Txscreen_index(ii))-CSK_x_screen)-(CSK_y(jj)-CSK_y_screen)/abs(CSK_x(jj)-CSK_x_screen))+abs(abs(y(symbol_Txscreen_index(ii))-CSK_y_screen)/(x(symbol_Txscreen_index(ii))-CSK_x_screen)-abs(CSK_y(jj)-CSK_y_screen)/(CSK_x(jj)-CSK_x_screen));
        end
        [kvalue_min, kindex_min]=min(delta_k);
        if kindex_min~=16
        symbol_received(symbol_Txscreen_index(ii)) = graycode_dec(kindex_min,:);
...
%% search for premble
premble_flag=0;
premble_number = 16; %4 for 500us; 8 for 1ms
premble_symbol = 8;
...
for ii=premble_index:length_min
        if length(symbol_received_recovered)==227
            tttt=1;
        end
...
msglen=8;
ecclen=4;
message_original_ASCII = [1,35,69,103,137,171,205,239];
...
%     temp=[];
%     temp=Decode_BER_16(message_encoded_ASCII((msglen+ecclen)*(ii-1)+1:(msglen+ecclen)*ii));
