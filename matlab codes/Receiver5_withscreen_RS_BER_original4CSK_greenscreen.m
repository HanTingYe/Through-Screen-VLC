...
count=12;
color='Results_RS_original4CSK_31_greenscreen_';
...
%         end
    ...
    if sqrt((x(symbol_Txscreen_index(ii))-CSK_x_screen)^2+(y(symbol_Txscreen_index(ii))-CSK_y_screen)^2)>Slope_threshold
        delta_k=[];
            for jj=1:M
                delta_k(jj)=abs((y(symbol_Txscreen_index(ii))-CSK_y_screen)/abs(x(symbol_Txscreen_index(ii))-CSK_x_screen)-(CSK_y(jj)-CSK_y_screen)/abs(CSK_x(jj)-CSK_x_screen));
            end
            [kvalue_min, kindex_min]=min(delta_k);
        switch(kindex_min)
        case 1 %not specific number, just an order
...
%% search for premble
premble_flag=0;
premble_number = 8; %4 for 500us; 8 for 1ms; 24 for intensity 31
