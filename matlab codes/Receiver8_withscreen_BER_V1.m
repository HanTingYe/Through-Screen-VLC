...
load Test_screen_intensity_16CSK_31.mat
...
%     delta_k=[];
%         if symbol_Txscreen_index(ii)==147
%             tttt=1;
%         end
%     for jj=1:M-1
...
%%  slope detection calibration V1 0.295 for intensity 15
% symbol_test=[];
% symbol_test=symbol_received;
% T_screen=diff(symbol_Txscreen_index);
...
% for ii=1:length(symbol_Txscreen_index)
%     delta_k=[];
%     if symbol_Txscreen_index(ii)==163
%         tttt=1;
%     end
...
for ii=1:length(symbol_received_recovered)
    bit_received_recovered = [bit_received_recovered, graycode_bin(symbol_received_recovered(ii)+1,:)];
end
bit_original=[];
for ii=1:length(symbol_original)
    bit_original = [bit_original, graycode_bin(symbol_original(ii)+1,:)];
end
