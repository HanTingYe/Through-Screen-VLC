...
for ii=1:length(symbol_Txscreen_index)
    delta_k=[];
    for jj=1:M-1
        delta_k(jj)=abs((x(symbol_Txscreen_index(ii))-CSK_x_screen)/(y(symbol_Txscreen_index(ii))-CSK_y_screen)-(CSK_x(jj)-CSK_x_screen)/(CSK_y(jj)-CSK_y_screen));
    end
    ...
    symbol_received(symbol_Txscreen_index(ii))=kindex_min-1;
    if delta_dis(ii)<0.02
...
for ii=premble_index:length_min
    if symbol_received(ii) ~= temp_com         
        if (symbol_received(ii)-temp_com) ==1 || symbol_received(ii)==0 || count_flag == 3 && symbol_received_recovered(end)~=symbol_received(ii)
            symbol_received_recovered = [symbol_received_recovered, symbol_received(ii)];
            symbol_index = [symbol_index, ii];
            if count_flag == 3
                temp_com = temp_com+1;
            else
    ...
    if count_flag==2 %   3 for 1ms
        symbol_index(end) = ii-1;
    end
end

% for ii=premble_index:length_min
%     if count_flag == 1 %   3 for 1ms
%         symbol_received_recovered = [symbol_received_recovered, symbol_received(ii)];
%         symbol_index = [symbol_index, ii];
%     end
%     if count_flag == 4
