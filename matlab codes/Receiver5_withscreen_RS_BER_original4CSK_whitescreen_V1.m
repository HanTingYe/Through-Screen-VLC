...
%%
load Results_RS_original4CSK_15_whitescreen_3.mat
premble_index=1456;
p_flag=0;%1 represents cal preamble; otherwise 0
...
%     if x(jj)<0.15&&y(jj)>0.12
%         error_index=[error_index jj];
%     end
% end
% 
% all_index=find(x)
% %error_index=find(Y<1000)
...
M = 4;
%V1
% CSK_x = [0.310,0.185,0.639,0.146];
% CSK_y = [0.379,0.667,0.328,0.07];

%V2
% CSK_x = [0.326,0.188,0.639,0.161];
% CSK_y = [0.370,0.679,0.328,0.077];

%white screen
CSK_x = [0.298,0.176,0.659,0.157];
CSK_y = [0.340,0.667,0.298,0.076];
% CSK_x_screen = 0.654400000000000;
% CSK_y_screen = 0.320500000000000;
CSK_x_screen = 0.3503;%0.665
CSK_y_screen = 0.3757;%0.3
symbol=[];
for ii=1:length_min
    delta_d=[];
    for jj=1:M
...
for ii=1:length_min
    switch(symbol(ii))
        case 0 %not specific number, just an order
            bit_received = [bit_received, 0,0];
        case 1
            bit_received = [bit_received, 0,1];
        case 2
            bit_received = [bit_received, 1,1];
        case 3
            bit_received = [bit_received, 1,0];
    end
end
...
screen_threshold=0;%mean(X(premble_index:length_min)) 0 for intensity 31
...
% % CSK_x_underscreen = [0.62, 0.602, 0.655, 0.527];
% % CSK_y_underscreen = [0.317, 0.352, 0.31, 0.245];
% % CSK_x_underscreen = [0.417, 0.399, 0.459, 0.35];
% % CSK_y_underscreen = [0.511, 0.550, 0.505, 0.395];
% % CSK_x_underscreen = [0.417, 0.399, 0.459, 0.34];
% % CSK_y_underscreen = [0.511, 0.550, 0.505, 0.375];
% 
% %whitescreen
% CSK_x_underscreen = [0.3503, 0.3399, 0.3813, 0.3061];
% CSK_y_underscreen = [0.3757, 0.4043, 0.3723, 0.3031];
% 
% T_screen=diff(symbol_Txscreen_index);
...
% for ii=1:length(symbol_Txscreen_index)
%     delta_k=[];
%             if symbol_Txscreen_index(ii)==1269
%                 tttt=1;
%             end
%     
%     
%     if sqrt((x(symbol_Txscreen_index(ii))-CSK_x_screen)^2+(y(symbol_Txscreen_index(ii))-CSK_y_screen)^2)>Slope_threshold
%         delta_k=[];
%         for jj=1:M-1
...
if (p_flag==1)
    ...
    premble_location=find(symbol_received==3)
...
for ii=premble_index:length_min
    if length(symbol_received_recovered)==69
        tttt=1;
    end
    ...
    if count_flag == 2 %
        ...
        if symbol_received(ii)==temp_com && symbol_received(ii-1)==temp_com  %排除5个一样的符号被判定为两个符号
            count_flag=-1;
            %             if ii<length_min && symbol_received(ii+1)==temp_com
            %                 count_flag=-2;
            %             end
        else
            count_flag=0;
...
for ii=1:length(symbol_received_recovered)
    switch(symbol_received_recovered(ii))
        case 0 %not specific number, just an order
            bit_received_recovered = [bit_received_recovered, 0,0];
        case 1
            bit_received_recovered = [bit_received_recovered, 0,1];
        case 2
            bit_received_recovered = [bit_received_recovered, 1,0];
        case 3
            bit_received_recovered = [bit_received_recovered, 1,1];
    end
end
bit_original=[];
for ii=1:length(symbol_original)
    switch(symbol_original(ii))
        case 0 %not specific number, just an order
            bit_original = [bit_original, 0,0];
        case 1
            bit_original = [bit_original, 0,1];
        case 2
            bit_original = [bit_original, 1,0];
        case 3
            bit_original = [bit_original, 1,1];
    end
end
...
msglen=12;
ecclen=6;
message_original_ASCII = [27,27,27,27,27,27,27,27,27,27,27,27];%163,170,189,173,173,63
...
for ii=1:fix(length(message_encoded_ASCII)/(msglen+ecclen))
    temp=[];
    temp=Decode_BER_4(message_encoded_ASCII((msglen+ecclen)*(ii-1)+1:(msglen+ecclen)*ii));
