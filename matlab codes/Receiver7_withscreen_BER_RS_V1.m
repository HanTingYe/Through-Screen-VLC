...
% 32CSK V2
% CSK_x = [0.1875, 0.1795, 0.2131, 0.1730, 0.2407, 0.1698, 0.2031, 0.1639, 0.25, 0.1594, 0.2187, 0.2388, 0.2820, 0.2794, ];
% CSK_y = [0.6667, 0.5897, 0.4262, 0.4230, 0.3889, 0.3585, 0.3281, 0.2951, 0.3166, 0.2029, 0.2187, 0.1343, 0.1538, 0.2353, ];
...
for ii=premble_index:length_min
    ...
    if count_flag == 2 %
        ...
        symbol_index = [symbol_index, ii-1];
        symbol_flag=0;
        if symbol_received(ii)==temp_com && symbol_received(ii-1)==temp_com %|| symbol_received(ii+1)==temp_com)  %排除5个一样的符号被判定为两个符号
            count_flag=-1;
        else
            count_flag=0;
        end
