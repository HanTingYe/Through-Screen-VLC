...
load Results_RS_8CSK_15_15cm_5.mat
...
%CSK_x = [0.381942857142857,0.188576022289933,0.401307346709709,0.214136614140756,0.477260268541437,0.310811373701920,0.173615146256616,0.576400000000000];
%CSK_y = [0.548100000000000,0.578542622160750,0.403195840982403,0.376153382924473,0.309090389478264,0.231160699731439,0.0915373448088586,0.317900000000000];

%CSK_x = [0.3676,0.2042,0.3900,0.2142,0.4657,0.3057,0.1729,0.5764];
%CSK_y = [0.4999,0.5835,0.4005,0.3876,0.2999,0.2306,0.1018,0.3179];

% CSK_x = [0.35,0.1875,0.3684,0.2308,0.4286,0.2727,0.1739,0.5764];
% CSK_y = [0.5,0.5625,0.4211,0.3846,0.2857,0.1818,0.0869,0.3179];

%15cm
% CSK_x = [0.349,0.194,0.412,0.219,0.487,0.303,0.16,0.625];
% CSK_y = [0.535,0.581,0.412,0.375,0.297,0.212,0.08,0.325];

CSK_x = [0.366,0.185,0.417,0.219,0.5,0.314,0.176,0.659];
CSK_y = [0.536,0.592,0.389,0.375,0.295,0.228,0.078,0.309];



% CSK_x_screen = 0.654400000000000;
% CSK_y_screen = 0.320500000000000;
CSK_x_screen = 0.66;%0.665 0.655 0.66
CSK_y_screen = 0.3;%0.3 0.31 0.3
symbol=[];
for ii=1:length_min
    delta_d=[];
    for jj=1:M
...
% CSK_x_underscreen = [0.649, 0.644, 0.652, 0.648, 0.659, 0.652, 0.634, 0.665];%647
% CSK_y_underscreen = [0.315, 0.312, 0.305, 0.303, 0.298, 0.295, 0.284, 0.298];%303

% CSK_x_underscreen = [0.649, 0.645, 0.652, 0.645, 0.655, 0.649, 0.626, 0.661];%647
% CSK_y_underscreen = [0.317, 0.315, 0.306, 0.306, 0.303, 0.300, 0.286, 0.304];%303


% CSK_x_underscreen = [0.645, 0.643, 0.649, 0.643, 0.651, 0.647, 0.628, 0.655];%647
% CSK_y_underscreen = [0.319, 0.317, 0.312, 0.310, 0.307, 0.303, 0.292, 0.311];%303

%CSK_x_underscreen = [0.645, 0.643, 0.649, 0.643, 0.653, 0.647, 0.628, 0.655];
%CSK_y_underscreen = [0.319, 0.317, 0.312, 0.310, 0.304, 0.303, 0.292, 0.311];

% CSK_x_underscreen = [0.649, 0.645, 0.652, 0.645, 0.655, 0.649, 0.626, 0.661];%647
% CSK_y_underscreen = [0.316, 0.313, 0.306, 0.306, 0.303, 0.300, 0.286, 0.304];%303

% CSK_x_underscreen = [0.639, 0.633, 0.645, 0.635, 0.651, 0.64, 0.616, 0.66];%647 633
% CSK_y_underscreen = [0.324, 0.321, 0.312, 0.308, 0.302, 0.298, 0.283, 0.31];%303 321

% CSK_x_underscreen = [0.627, 0.620, 0.638, 0.619, 0.644, 0.628, 0.590, 0.659];%647 633
% CSK_y_underscreen = [0.332, 0.329, 0.315, 0.311, 0.303, 0.295, 0.271, 0.306];%303 321

CSK_x_underscreen = [0.633, 0.625, 0.640, 0.623, 0.646, 0.630, 0.6, 0.659];%647 633
CSK_y_underscreen = [0.328, 0.325, 0.314, 0.310, 0.303, 0.296, 0.276, 0.307];%303 321

T_screen=diff(symbol_Txscreen_index);
Slope_threshold=0.075;%0.075
delta_dis=[];
for ii=1:length(symbol_Txscreen_index)
    delta_k=[];
        if symbol_Txscreen_index(ii)==1319
            tttt=1;
        end
...
%% search for premble
premble_flag=0;
premble_number = 16; %4 for 500us; 8 for 1ms; 24 for intensity 31
